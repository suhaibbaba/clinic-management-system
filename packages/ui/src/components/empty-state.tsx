...
export function EmptyState({ title, hint, action, icon = 'file' }: EmptyStateProps): JSX.Element {
  ...
  return (
    ...
    >
      ...
      <p data-part="empty-state-title" className="text-section font-medium text-ink">
        {t(title)}
      </p>
      {hint !== undefined && (
        <p data-part="empty-state-hint" className="mt-1 text-label text-ink-muted">
          {t(hint)}
        </p>
      )}
      {action !== undefined && (
        <div data-part="empty-state-action" className="mt-4 flex justify-center">
          {action}
        </div>
      )}
    </div>
