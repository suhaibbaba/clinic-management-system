...
// Three questions a real browser has to answer: does it render for the entitled role, is it laid
// out in the right direction, and does it fit. The third is why this is in CI.
...
for (const role of ROLES) {
  test.describe(`${role}`, () => {
    // Addresses only. States behind a click belong to the visual sweep; this has to stay fast
...
// Read out of the app rather than the API: the access token is held in memory, so the honest way to
// find a record is to open the list and pick a row.
async function resolvePath(page: Page, path: string): Promise<string> {
  ...
  // The way in differs by list, so try the row and then the first control inside it — which covers
