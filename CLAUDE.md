...
- **Every interactive control is `--control-h` or `--control-h-sm`; a third height requires changing the tokens, never an inline value.** 44px is the tap target WCAG 2.5.8 asks for and the height the reference draws a field at; 32px is the compact row a laptop gets for a chip, a tab, a segment or a table-row button. Below `lg` a compact control is still the tall one, because a thumb does not shrink with the viewport. Both scales share one anatomy: 10px radius (full for a pill), 16px text (13.5px on the small one), and an 8px gap between a control's icon and its words.
...
- **Every phone number dials and every email opens a message** — `<PhoneLink>` and `<EmailLink>` from `components/ui`, never bare text. Reception's job is largely ringing people, and a number they have to read off the screen and key into a handset is a number the app did not help with. `tel:` gets the digits stripped of spacing; the visible text stays exactly as it was entered, because that is what gets read out loud. Both are `<Ltr>` islands and both draw a dash rather than an empty link when the field is null. Printed documents are the exception: a link on paper is nothing.
...
- **Every amount is `<Money>`, every price field is `<MoneyInput>`, every staff name is `<PersonName>`.** They live in `components/ui` because inventory, labs, the dashboard and patients draw them as much as billing does, and a shared control inside one feature is a control other features import across a boundary they should not.
