...
- **A dialog focuses nothing when it opens.** Radix's default is the first focusable element, which in this app is usually a date field — a caret in an untouched form, and a calendar unfolding over it. `Modal` and `Drawer` prevent `onOpenAutoFocus` and focus the container instead; the trap, Escape and the first Tab are unchanged. The root cause is fixed alongside it: `PopoverSheet` **anchors** rather than triggers, so a picker opens only on an explicit click, Enter, Space or ArrowDown — never on focus. A scrollable dialog body carries enough inner padding for a full focus ring (3px plus a 2px offset).
- **`Select` is the platform's control on a pointer device and ours on a phone.** A native `<select>` is the right answer on a desktop — it mirrors in RTL, it is keyboard accessible with no code of ours, and a test can read its options. It stopped being the right answer on an iPhone: reported from a clinic, tapping a dropdown in Safari did nothing at all, on every screen, with the field present, enabled, uncovered and populated. A native picker is not part of the page, so that failure can be neither reproduced nor regression-tested anywhere but on the device — which is the reason to stop depending on it rather than to keep guessing at it. Below `md` the same options open in the bottom sheet `PopoverSheet` already draws there. Both shapes take the same props and emit the same `event.target.value`, so no caller knows which one it has.
