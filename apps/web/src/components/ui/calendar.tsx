...
function CalendarDayButton({
  ...
  return <button ref={ref} {...props} className={cn(className, dayInk(modifiers))} />;
}
...
function CalendarHeader({ label, zoomOut, previous, next }: CalendarHeaderProps): JSX.Element {
  return (
    <div className="flex h-(--control-h-sm) items-center justify-between gap-2">
      <button
        type="button"
      ...
      {zoomOut ? (
        ...
        <span className="text-value font-medium text-ink">{label}</span>
      )}
